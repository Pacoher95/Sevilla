...
# Archivo CSV persistente
csv_path = "datos_usuarios.csv"
...
# Guardar en CSV compartido
if st.button("Enviar y ver resultados"):
    if os.path.exists(csv_path):
        datos_previos = pd.read_csv(csv_path)
        datos_nuevos = pd.concat([datos_previos, usuario], ignore_index=True)
    else:
        datos_nuevos = usuario
    datos_nuevos.to_csv(csv_path, index=False)
    st.success("✅ ¡Respuesta registrada!")

# ------------------------
# 2. Clustering y análisis colectivo
# ------------------------
if os.path.exists(csv_path):
    ...
    if len(data) >= 6:
        ...
        st.caption("Clustering realizado con K-Means + PCA. Visualización dinámica con las respuestas acumuladas.")

        # Mostrar todas las respuestas
        with st.expander("👥 Ver respuestas individuales registradas"):
            st.dataframe(data.drop(columns=["PCA1", "PCA2"]))

        # Opción de descarga
        csv = data.to_csv(index=False).encode('utf-8')
        st.download_button("📥 Descargar respuestas en CSV", csv, "respuestas_gimnasio.csv", "text/csv")
    else:
        st.warning("Se necesitan al menos 6 respuestas para activar el análisis grupal. Comparte esta encuesta con más asistentes.")
else:
    st.info("Esperando las primeras respuestas...")
